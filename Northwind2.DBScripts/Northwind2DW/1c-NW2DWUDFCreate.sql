...
GO
CREATE FUNCTION [dbo].[udf_BuildDateFromDateParts] (@intMonth INTEGER
                                                    ,@intDay  INTEGER
                                                    ,@intYear INTEGER)
RETURNS DATETIME2
AS
  BEGIN
      ...
      SET @strYYYYMMDD = CAST(@intYear AS CHAR(4)) + '/' + RTRIM(CAST(@intMonth AS CHAR(2))) + '/' + RTRIM(CAST(@intDay AS CHAR(2)))
...
*/
CREATE FUNCTION [dbo].[udf_Trim] (@StringToClean AS VARCHAR(8000))
